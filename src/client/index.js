...
(function() {
  ...
  var LIVE_EDIT_CONFIG_KEY = "devtools-live-config";

  /**
	 * DevtoolsLive client controller.
	 *
	 * @class DevtoolsLiveClient
	 * @private
	 */

  function DevtoolsLiveClient() {
    var self = this;
    loadConfig(function(config) {
      ...
      self.logger = self.createLogger("devtools-live");
      self.createPanel();
      self.start();
    });
  }
  ...
  DevtoolsLiveClient.prototype.saveConfig = function() {
  ...
  DevtoolsLiveClient.prototype.listenToPanel = function(type, callback) {
    if (!this.panelWindow) {
  ...
  DevtoolsLiveClient.prototype.triggerEvent = function(type, data) {
  ...
  DevtoolsLiveClient.prototype.initPanel = function() {
  ...
  /**
	 * Starts the DevtoolsLive client.
	 *
	 * @private
	 */

  DevtoolsLiveClient.prototype.start = function() {
    this.status("starting");
    this.startNewSession();
  };

  /**
	 * Stops DevtoolsLive client.
	 *
	 * @private
	 */

  DevtoolsLiveClient.prototype.stop = function() {
  ...
  DevtoolsLiveClient.prototype.getLocation = function(callback) {
  ...
  DevtoolsLiveClient.prototype.getSite = function(host) {
  ...
  DevtoolsLiveClient.prototype.startNewSession = function() {
    if (this.session) {
      this.stop();
    }
  ...
  /**
	 * Enables DevtoolsLive for the current inspected window host.
	 *
	 * @private
	 */

  DevtoolsLiveClient.prototype.enableForHost = function() {
    this.getLocation(function(host) {
  ...
  DevtoolsLiveClient.prototype.status = function(status, aux) {
    var text, action;
    switch (status) {
      case "starting":
        text = "Starting";
  ...
  function tryToLoadLegacyConfig() {
    ...
    try {
      config = window.localStorage &&
        localStorage.getItem(LIVE_EDIT_CONFIG_KEY);
      if (config) {
