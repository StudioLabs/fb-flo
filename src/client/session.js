			...
			resource.setContent(update.content, true, function(status) {
				if (status.code != 'OK') {
					...
					if (update.event !== undefined) {
						if (update.resourceName !== undefined && !this.openedSources[update.resourceName]) {
							chrome.devtools.panels.openResource(this.devResources[update.resourceName].url, null, function() {
