...
LiveEdit.prototype.onFileChange = function(filepath,root) {
    ...
    var fileUrl = filepath.replace(this.src + '/', '');
    this.log('File changed',filepath, fileUrl);
    if(this.resolvers[extension] !== undefined){
      this.resolvers[extension].resolve(filepath, fileUrl, this);
    }else{
      this.resolve(filepath, fileUrl) ;
    }
};
...
/**
 * Start server, watchers an load resolvers
 *
 * @private
 */
LiveEdit.prototype.start = function() {
  
  this.log = logger(this.options.verbose, 'LiveEdit');

  this.server = new Server({
    port: this.options.port,
    log: logger(this.options.verbose, 'Server'),
    parent : this
  });
